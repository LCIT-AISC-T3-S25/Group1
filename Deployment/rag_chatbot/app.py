...
def initialize_rag_system():
    ...
    try:
        ...
        if hasattr(chatbot, "initialize_system"):
            ...
            logger.info(
                "No explicit initialization function found, checking for global variables..."
            )
            # Try to trigger initialization by accessing global variables
            if (
                hasattr(chatbot, "passages")
                and hasattr(chatbot, "embedder")
                and hasattr(chatbot, "faiss_index")
            ):
        ...
        if hasattr(chatbot, "medical_chatbot"):
            ...
            logger.info(
                f"Available functions: {[name for name in dir(chatbot) if not name.startswith('_')]}"
            )
            return False
...
def predict():
    ...
    try:
        ...
        logger.info(
            f"Processed query: {query[:50]}... | Confidence: {result['confidence']}"
        )
...
def model_info():
    ...
    try:
        info = {
            ...
            "input_format": {"query": "string - Medical question/query"},
            "output_format": {
